...
class ChildsNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val childsNameRoute = routes.ChildsNameController.onPageLoad(NormalMode).url
  ...
  "ChildsName Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, childsNameRoute)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, childsNameRoute)
