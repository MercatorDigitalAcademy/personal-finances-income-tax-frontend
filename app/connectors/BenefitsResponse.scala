...
object BenefitsResponse {
  ...
  implicit val benefitsResponseReads: HttpReads[BenefitsResponse] =
    new HttpReads[BenefitsResponse] with Parser with Logging {
      ...
      override protected[connectors] val parserName: String =
      ...
      override def read(
        ...
        response.status match {
          ...
          case NOT_FOUND | INTERNAL_SERVER_ERROR | SERVICE_UNAVAILABLE |
              BAD_REQUEST =>
      ...
      private def extractResult(
          response: HttpResponse
      ): Either[ApiError, FetchedBenefits] =
        response.json
          .validate[FetchedBenefits]
          .fold[Either[ApiError, FetchedBenefits]](
            e => {

              logger.error(
                s"[BenefitsResponse][extractResult]: Error parsing benefits submission JSON: $e"
              )
